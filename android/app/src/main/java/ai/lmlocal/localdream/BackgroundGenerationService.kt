...
class BackgroundGenerationService : Service() {
    ...
    private suspend fun runGeneration(
        ...
        try {
            ...
            wakeLock = powerManager.newWakeLock(android.os.PowerManager.PARTIAL_WAKE_LOCK, "LocalDream:GenerationWakeLock")
            wakeLock.acquire(10 * 60 * 1000L) // 10 minutes max timeout exactly so it never leaks
            Log.i("BgGenService", "WakeLock acquired to prevent NPU timeout")

            updateState(GenerationState.Progress(0f))

            // Pre-flight: verify backend is alive before attempting generation
            ...
            for (attempt in 1..5) {
                try {
                    val healthReq = Request.Builder()
                        ...
                        .get()
                        .build()
                    val healthResp = healthClient.newCall(healthReq).execute()
                    if (healthResp.isSuccessful) {
                        backendAlive = true
                        healthResp.close()
                        Log.i("BgGenService", "✅ Backend health check passed (attempt $attempt)")
                        break
                    }
                    healthResp.close()
                } catch (e: Exception) {
                    Log.w("BgGenService", "⏳ Backend not ready yet, attempt $attempt/5: ${e.message}")
                }
                if (attempt < 5) {
                    delay(1000L * attempt) // Backoff: 1s, 2s, 3s, 4s
                }
            }
            if (!backendAlive) {
                throw IOException("El servidor backend no está respondiendo. Recarga el modelo e intenta de nuevo.")
            }
            ...
            // Retry the generation POST with exponential backoff
            ...
            for (attempt in 1..3) {
                try {
                    ...
                    if (response.isSuccessful) {
                        Log.i("BgGenService", "✅ Generation POST succeeded on attempt $attempt")
                        break
                    } else {
                        val code = response.code
                        response.close()
                        response = null
                        throw IOException("Error en la solicitud: $code")
                    }
                } catch (e: Exception) {
                    lastException = e
                    Log.w("BgGenService", "⚠️ Generation POST failed (attempt $attempt/3): ${e.message}")
                    if (attempt < 3) {
                        val backoffMs = 1000L * (1 shl (attempt - 1)) // 1s, 2s
                        Log.d("BgGenService", "Retrying in ${backoffMs}ms...")
                        delay(backoffMs)
                    }
                }
            }
            ...
            if (response == null || !response.isSuccessful) {
                throw lastException ?: IOException("Falló la conexión al backend después de 3 intentos")
            }
            ...
            if (wakeLock?.isHeld == true) {
                wakeLock.release()
                Log.i("BgGenService", "WakeLock released")
            }
        }
    }
    ...
    private fun createNotification(progress: Float): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            ...
            .setContentText("Progreso: ${(progress * 100).toInt()}%")
