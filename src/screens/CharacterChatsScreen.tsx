...
type RouteProps = RouteProp<RootStackParamList, 'CharacterChats'>;
...
export const CharacterChatsScreen: React.FC = () => {
  ...
  const handleNewChat = () => {
    if (!hasModels) {
      setAlertState(showAlert('Sin Modelo', 'Por favor descarga un modelo de IA en la pestaña de Modelos.'));
      return;
    }
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle} numberOfLines={1}>
            {character?.name || 'Historial de Chats'}
          </Text>
        </View>
      ...
      {characterChats.length === 0 ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyText}>
            {hasModels
              ? 'Empieza una conversación con este personaje.'
              : 'Descarga un modelo para empezar a chatear.'}
          </Text>
          {hasModels && (
            <Button
              title="Iniciar chat"
